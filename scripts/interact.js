...
async function stakingInteraction() {
    ...
    console.log("TON balance before swap ", ethers.utils.formatUnits(await TON.balanceOf(deployerAddress),"ether"));
    
    //SWAP 20 WTO FOR 20 TON IN RAY FORMAT!
    await WTON.swapToTON("200000000000000000000000000000");
    console.log("TON balance after swap ",  ethers.utils.formatUnits(await TON.balanceOf(deployerAddress), "ether"));
    console.log();
    ...
    let secondPart = unmarshalString(padLeft(unmarshalString(Tokamak1Layer2Address),64));
    const data = firstDataPart+secondPart
    const amountInWeiForApproveAndCall = ethers.utils.parseEther("100");
    console.log("===============Deposit Manager contract data before call approveAndCall===============");
    ...
    console.log();
    console.log("===============CALL approveAndCall FROM TON===============");
    await TON.approveAndCall(WTONMainnetAddress, amountInWeiForApproveAndCall, data);
    console.log();
    ...
    console.log("User WTON balance after deposit:", ethers.utils.formatUnits(coinageBalance, 27));
    ...
    console.log("User WTON balance in wallet balance before processing withdrawal:", ethers.utils.formatUnits(walletWTONAmountBeforeWithdraw, 27));
    //Cover DTD
